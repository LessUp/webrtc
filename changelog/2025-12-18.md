...
- 前端：实现 Mesh 多人通话（多 PeerConnection 管理）：为每个远端成员维护独立 `RTCPeerConnection`/`RTCDataChannel`，并按 `from/to` 分发处理 `offer/answer/candidate`。
- 前端：多路视频布局：按 `peerId` 动态创建/移除远端视频卡片（`<video>`），并在 `ontrack` 时绑定远端媒体流。
- 前端：屏幕共享与录制适配多人：屏幕共享时对所有连接执行 `replaceTrack`；录制优先选择任意远端流，其次屏幕/本地流。
- 文档：在 `ROADMAP.md` 中勾选阶段 3 的“多 PeerConnection 管理 / 多路视频布局”为已完成。
- 文档：同步更新 `docs/guide.md`，将单对单 `pc/remoteVideo/dataChannel` 的说明改为 Mesh 多人通话（`peers` 映射 + 动态视频布局 + 广播聊天）。
